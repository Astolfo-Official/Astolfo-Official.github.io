...
这一节直接给定简化的 Fabry-Perot 边界条件：纵向镜间距为 $\ell_z$，两面无损镜相同，镜子的透射振幅为 $T\ll 1$。如果需要把横向动量先离散化，可以取横向量子化长度为 $\ell_x$ 和 $\ell_y$，横向面积为 $A=\ell_x\ell_y$。有限面积中允许的面内波矢是离散的，
...
如果外场作用在整个平面上，并且在腔平面内具有确定的平面波因子 $e^{i\mathbf{k}_{\parallel}\cdot\mathbf{r}}$，即是在整个平面上具有相同相位的横向场，那么它只选择一个面内动量模式 $\mathbf{k}=\mathbf{k}_{\parallel}$。此时每个被选中的模式都退化成单模问题，最终结果就是把单模公式中的 $\omega_c$ 换成该模式的 $\omega_{\mathbf{k}}$：
